...
def _is_full_capture_crop(crop: CaptureCrop | None) -> bool:
    if crop is None:
        return True
    return crop.x <= 0.001 and crop.y <= 0.001 and crop.width >= 0.999 and crop.height >= 0.999
...
def _rel(abs_path: str | Path, user_path: Path) -> str:
    ...
    return Path(abs_path).relative_to(settings.storage_path).as_posix()
...
async def upload_image(
    ...
    if station:
        ...
        station.last_used_at = datetime.utcnow().isoformat()
...
async def set_corners(request: Request, session_id: str, req: CornersRequest, user_id: str = Depends(get_user_id)):
    ...
    station_name = (req.save_station_name or "").strip()
    if req.save_station_name is not None:
        image_width, image_height = _session_image_dimensions(session)
        now = datetime.utcnow().isoformat()
        station_id = str(uuid.uuid4())
        created_station = PhotoStation(
            id=station_id,
            name=station_name or f"Station {now[:10]}",
            image_width=image_width,
            image_height=image_height,
            image_path=_copy_station_image(user_id, orig_path, station_id),
        ...
        get_photo_station_store(user_id).set(created_station.id, created_station)
...
async def create_photo_station(request: Request, req: PhotoStationCreateRequest, user_id: str = Depends(get_user_id)):
    ...
    now = datetime.utcnow().isoformat()
    station_id = str(uuid.uuid4())
    source_image = _abs(session.original_image_path) or _abs(session.station_image_path)
    station = PhotoStation(
        id=station_id,
        name=req.name.strip() or f"Station {now[:10]}",
        image_width=image_width,
        image_height=image_height,
